...
// 全部走 chatBase（copilot.tencent.com）+ BillingHeaders，信封同 doJSON。
// 移植自 workbuddy2api 上游（79 提交领先版），与 report.go 组成每日积分增益闭环。
package upstream
...
func (c *Client) growthJSON(a *auth.Auth, method, path string, body any) (json.RawMessage, error) {
	...
	req, err := http.NewRequest(method, c.chatBase(a.Snapshot().Region)+path, rdr)
	if err != nil {
		return nil, err
	}
